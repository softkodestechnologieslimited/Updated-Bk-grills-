...
const StockItemCard = () => {
  ...
  const getMeal = async () => {
    ...
    const { title, description, category, price, quantity, inStock, image, imageId } = meal;
    setItemDetails({ title, description, category, price, quantity, inStock, imageId })
    setImageUrl(image)
  }
  ...
  const { title, description, category, price, quantity, addQuantity, inStock, imageId } = itemDetails;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiService.updateMeal({ id, title, description, category, price, quantity: newQuantity, inStock, imageUrl, imageId });
        ...
        <div className="relative flex flex-col xl:w-8/12 mx-auto min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-gray-200 border-0">
          <div className="rounded-t bg-white mb-0 px-6 py-6">
            <div className="text-center flex justify-between">
              <h6 className="text-gray-800 text-xl font-bold">Edit Menu Item</h6>
              <Link
            ...
            <form onSubmit={handleSubmit} className="flex-auto p-5 lg:p-10">
              ...
              <div className="relative w-full mb-3">
                <label className="block uppercase text-gray-700 text-xs font-bold mb-2">
                  ...
                  <select className="form-select block w-full my-4 p-3" onChange={handleChange} name='category' value={category}>
                    <option value=''
              ...
              <div className="relative w-full mb-3">
                <label className="block">
                  ...
                  <input className="form-input text-gray-700 mt-1 block w-full my-4 p-3" placeholder="Quantity" name='addQuantity' value={addQuantity}
                    onChange={handleChange}
                  />
                </label>
              </div>
              ...
              <div className="flex mt-6">
                <label className="flex items-center">
                  <input type="checkbox" className="form-checkbox text-green-500" checked={inStock || ''} onChange={handleCheck} />
