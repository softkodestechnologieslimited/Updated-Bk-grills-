...
const ClockInService = ({children})=>{
  ...
  useEffect(() => {
    ...
    useEffect(()=>{
      ...
      newArray = newArray.filter(item => item.ticket_id.indexOf(ticketParams.toLowerCase()) > -1).sort((a, b)=> a-b)
  ...
  useEffect(()=>{
    ...
    newArray = newArray.filter(item => item.name.toLowerCase().indexOf(filterText.toLowerCase()) > -1).sort((a, b)=> a-b)
  ...
  const fetchTickets= async (timeStamp)=>{
    ...
    try {
      const data = await axios.get(`${baseApiUrl}${apiFormat}&timestamp=${Math.floor(timeStamp/1000)}`)
  ...
  return (
    <ClockedInContext.Provider value={{testStaff, setTestStaff, ClockInStaff, clockedInStaff, filteredClock, handleSearch, addTicket, handleTicketSearch, handleDateChange, startDate, setStartDate, fetchTickets, filteredTickets, loading, deleteTicket}}>
      {children}
