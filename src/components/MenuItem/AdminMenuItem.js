...
const AdminMenuItem = observer(({ meal }) => {
  ...
  const drinksPlaceholderImage = 'https://images.unsplash.com/photo-1582106245687-cbb466a9f07f?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1050&q=80';

  const quantityInCart = () => {
    const itemInCart = cartService.meals.find(cartMeal => meal.id === cartMeal.id);
  ...
  const addQuantity = () => {
    ...
    const itemInCart = cartService.meals.find(cartMeal => meal.id === cartMeal.id);
    itemInCart.quantity !== 1 ? (addToast("Item updated", {
      appearance: 'info',
      autoDismiss: true,
  ...
  const reduceQuantity = () => {
    ...
    const itemInCart = cartService.meals.find(cartMeal => meal.id === cartMeal.id);
    itemInCart ? (addToast("Item updated", {
      appearance: 'info',
      autoDismiss: true,
      autoDismissTimeout: 2000
        ...
        <div className="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded-lg bg-transparent">
          {
            ...
            />) : (
              <Image
                alt="..."
                src={meal.category === "food" ? foodPlaceholderImage : drinksPlaceholderImage}
                className="w-full rounded-t-lg"
              />
            )
          }
          ...
          <blockquote className="relative p-3">
            ...
            <div className="mt-5">
              ...
              <span className="text-gray-800 px-4 font-bold">{quantityInCart()}</span>
              <button
