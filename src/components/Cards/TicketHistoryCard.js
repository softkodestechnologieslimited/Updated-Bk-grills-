...
const TicketHistoryCard = (props) => {
  ...
  const { handleDateChange, startDate, loading, setStartDate, deleteTicket } =
    useClockedInContext();
          ...
          <div className="flex py-2 flex-wrap items-center">
            ...
            <div className="relative py-2 w-full px-4">
              <h3 className="font-semibold text-base text-center text-gray-800">
                Showing ticket history for {formatLongDate(startDate)}{" "}
              </h3>
            </div>
            <div className="relative py-2 w-full px-4">
              {ticket.length !== 0 && !loading && (
                <h3 className="font-semibold text-base text-gray-800">
                  No of Tickets: {ticket.length}
                </h3>
              )}
            </div>
            <br />
                  ...
                  <div className="body">
                    <div className="message">
                      <p className="no-align">
                        Ticket ID:<strong> {selectedTicket.ticket_id}</strong>{" "}
                      </p>
                      <p className="no-align">
                        Date Printed:{" "}
                        <strong>
                          {formatLongDate(
                            new Date(selectedTicket.timestamp).getTime()
                          )}
                        </strong>
                      </p>
                      <p className="no-align">
